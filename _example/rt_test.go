...
func loadEchoRoutes(m *mux.Router, routes []*Route) {
	for _, r := range routes {
		switch r.Method {
		case "GET":
			m.Get(r.Path, MinimaHandler(r.Method, r.Path))
		case "POST":
			m.Post(r.Path, MinimaHandler(r.Method, r.Path))
		case "PATCH":
			m.Patch(r.Path, MinimaHandler(r.Method, r.Path))
		case "PUT":
			m.Put(r.Path, MinimaHandler(r.Method, r.Path))
		case "DELETE":
			m.Delete(r.Path, MinimaHandler(r.Method, r.Path))
		}
	}
}

func MinimaHandler(method, path string) mux.Handler {
...
func BenchmarkMinimaStatic(b *testing.B) {
	m := mux.NewRouter()
	loadEchoRoutes(m, static)
	benchmarkRoutes(b, m, static)
}

func BenchmarkMinimaGitHubAPI(b *testing.B) {
	m := mux.NewRouter()
	loadEchoRoutes(m, githubAPI)
	benchmarkRoutes(b, m, githubAPI)
}

func BenchmarkMinimaGplusAPI(b *testing.B) {
	e := mux.NewRouter()
	loadEchoRoutes(e, gplusAPI)
	benchmarkRoutes(b, e, gplusAPI)
}

func BenchmarkMinimaParseAPI(b *testing.B) {
	m := mux.NewRouter()
	loadEchoRoutes(m, parseAPI)
